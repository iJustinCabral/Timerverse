...
@implementation SAAlarmControl{
    NSMutableArray *sectorsArray;
    
    SAAlarmSector *trackingSector;
...
- (BOOL) beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    for(NSUInteger i = 0; i < sectorsArray.count; i++){
        SAAlarmSector *sector = sectorsArray[i];
...
//            double diff = trackingSector.endValue - newValue;
            ...
            // This lets it progress around completlely.
...
- (void)drawSector:(SAAlarmSector *)sector atPosition:(NSUInteger)position{
...
- (SASectorDrawingInformation) sectorToDrawInf:(SAAlarmSector *)sector position:(NSInteger)position{
    ...
    if(markersCentresSegmentLength < markersRadiusSumm){
        ...
        drawInf.startMarkerAlpha = 0.0; // i changed this
    }
...
- (void) drawString:(NSString *)s withFont:(UIFont *)font color:(UIColor *)color withCenter:(CGPoint)center{
    ...
    NSDictionary *attributes = @{NSFontAttributeName: [font fontWithSize:14]};
    CGSize size = CGSizeMake(0, 0); // fix this in the future
